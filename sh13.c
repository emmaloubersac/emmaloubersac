...
char gbuffer[256];
int gServerPort;
char gServerIpAddress[100];
char gClientIpAddress[100];
int gClientPort;
char gName[55];
char gNames[4][55];
int gId;
...
void *fn_serveur_tcp(void *arg)
{
       ...
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1)
        {
                ...
                synchro=1; // un message est arrivé dans le réseau !!

                while (synchro); //attend que la requete soit traiter avant d'en accepter une autres (tant que synchro n'est pas a 0)
...
int main(int argc, char ** argv)
{
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
        ...
        if (synchro==1) //attention, un message est arrivé, traitement du message dans le graphique
        //boucle graphique a recu un message disant qu'un nouveau message réseau est arrivé
        {
                //printf("consomme |%s|\n",gbuffer);
		switch (gbuffer[0])
		{
